...
| **Orchestration**     | Airflow                              | Orchestration des pipelines           |
...
| **Documentation**     | mkdocs                               | Documentation technique               |
...
Le projet utilise **GitHub Actions** pour automatiser les vérifications de qualité à chaque push et pull request sur `main`.

### Workflow

Le fichier `.github/workflows/ci.yml` exécute trois étapes séquentielles :

1. **Lint** (`ruff check`) — détecte les erreurs de syntaxe, imports non triés, code mort, etc.
2. **Format check** (`ruff format --check`) — vérifie que le code respecte le formatage standard (sans le modifier). Un échec ici signifie qu'il faut lancer `uv run ruff format .` localement avant de commit.
3. **Tests** (`pytest`) — exécute la suite de tests unitaires.

Ces vérifications s'exécutent dans un environnement Ubuntu propre, avec les dépendances installées via `uv sync --all-extras --dev`.
...
- tout `push` vers `main`
- toute `pull_request` ciblant `main`

### Exécuter les vérifications en local

Avant de pousser du code, il est recommandé de reproduire les mêmes checks en local :
...
# Formatage (correction automatique)
uv run ruff format .
...
[tool.ruff.lint]
select = ["E", "F", "I"]  # erreurs, pyflakes, imports triés
```

### Statut du build

Ajouter le badge suivant en haut du README principal pour visualiser l'état du dernier run (remplacer `<owner>/<repo>`) :
