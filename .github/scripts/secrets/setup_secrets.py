...
class GitHubSecretsManager:
    ...
    def create_or_update_secret(self, secret_name: str, secret_value: str) -> bool:
        ...
        try:
            ...
            # Préparer le payload
            payload = {"encrypted_value": encrypted_value, "key_id": self.public_key_id}
...
def load_secrets_from_file(config_file: Path) -> Dict[str, str]:
    ...
    try:
        ...
        if not isinstance(secrets, dict):
            print("❌ Erreur: Le fichier doit contenir un objet JSON")
            return {}
...
def main():
    ...
    parser.add_argument(
        "--list", action="store_true", help="Lister les secrets existants sans en ajouter"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
