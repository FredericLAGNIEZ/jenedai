...
class ModelPerformanceDebugger:
    ...
    def __init__(
        ...
        self.X_train = (
            X_train
            if X_train is None or isinstance(X_train, pd.DataFrame)
        ...
        for col in self.X_test.columns:
            try:
                ...
                # For categorical features, analyze each category
                    ...
                    self.X_test[col]
                ) or pd.api.types.is_categorical_dtype(self.X_test[col]):
    ...
    def _calculate_segment_metrics(self, mask: np.ndarray) -> Dict:
        ...
        if self.task_type == "classification":
            ...
            metrics["f1_score"] = f1_score(
                y_true_seg, y_pred_seg, average="weighted", zero_division=0
            )
            ...
            try:
                if pd.api.types.is_numeric_dtype(self.X_test[col]):
                    ...
                    psi = np.sum(
                        ...
                        * np.log((test_dist + 1e-10) / (train_dist + 1e-10))
                    )
    ...
    def plot_feature_performance_breakdown(self, top_n: int = 10, save_path: Optional[str] = None):
        ...
        fig, axes = plt.subplots(
            min(5, len(top_features)), 1, figsize=(12, 4 * min(5, len(top_features)))
        )
        ...
        for idx, (feature, _) in enumerate(top_features[:5]):
            ...
            axes[idx].axhline(
                y=self.overall_metrics.get(
                    "accuracy" if self.task_type == "classification" else "mae"
                ),
                color="r",
                linestyle="--",
                label="Overall",
